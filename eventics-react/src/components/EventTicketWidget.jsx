	...
	return (
		<div className="et-event-details-ticket-widgget border border-[#e5e5e5] rounded-[16px] overflow-hidden">
			...
			<div className="p-[22px] lg:p-[16px]">
				...
				{/* <!-- slider --> */}
				<Swiper modules={[Navigation, FreeMode]} freeMode navigation={{ nextEl: "#et-event-details-ticket-time-slider-nav .next", prevEl: "#et-event-details-ticket-time-slider-nav .prev" }} slidesPerView={4.5} spaceBetween={10} className="et-event-details-ticket-time-slider swiper overflow-visible mb-[24px]">
					{timings.map((time, index) => (
						<SwiperSlide key={index} className="w-max group">
							<span onClick={() => handleTimeSelect(time)} className={`border border-[#e5e5e5] rounded-[4px] h-[30px] inline-flex items-center justify-center px-[15px] font-inter font-normal text-[14px] text-[#232323] cursor-pointer ${selectedTime === time ? "bg-etBlue border-etBlue text-white" : ""}`}>
								{time}
							</span>
						</SwiperSlide>
					))}
				...
				<form className="space-y-[10px] mb-[30px]">
					{levels.map((level, index) => (
						<div key={index} className="radio-container border border-[#d9d9d9] rounded-[6px] px-[16px] py-[7px]">
				...
				<div className="border-[0.5px] border-[#d9d9d9] px-[80px] xxs:px-[30px] rounded-full mb-[30px]">
					<div className="flex items-center justify-between gap-[15px] py-[17px]">
						<button onClick={decreaseTicket} type="button" id="decreaseButton" className="decrease font-extralight text-[35px] bg-etBlue/10 w-[28px] aspect-square rounded-full inline-flex items-center justify-center hover:bg-etBlue hover:text-white">
						...
						<button onClick={increaseTicket} type="button" id="increaseButton" className="increase font-extralight text-[35px] bg-etBlue/10 w-[28px] aspect-square rounded-full inline-flex items-center justify-center hover:bg-etBlue hover:text-white">
				...
				{/* <!-- button --> */}
				<button onClick={handleTicketPurchase} className="bg-etBlue h-[50px] rounded-full px-[15px] flex items-center justify-center gap-x-[10px] w-full text-white text-[15px] hover:bg-[#000D83]">
					<span>
						<img src="/assets/img/ticket-icon.svg" alt="ticket icon" />
					</span>
					<span>{selectedTicket.price * quantity} €-get Tickets Now</span>
				</button>
				{isModalOpen && (
					<div className="bg-black/90 w-full h-full fixed top-0 left-0 z-50" id="ticketModal">
						<div class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 scale-150 bg-white rounded-lg shadow-xl w-full mx-4 max-h-[90vh] overflow-auto bg-white dark:bg-gray-900 border-0 dark:border dark:border-gray-700 max-w-md">
							<div class="p-4">
