...
const EventTabs = () => {
	...
	const tabs = [
		{ tab: "day1", label: "Day 01" },
		{ tab: "day2", label: "Day 02" },
		{ tab: "day3", label: "Day 03" },
	];
						...
						return (
							...
							<div key={idx} id={tab.tab} className={`et-tab ${tab.tab == activeTab ? "active" : ""}`}>
								<div className="all-scheduled-events space-y-[30px]">
									{!schedules.filter((schedule) => schedule.day == tab.tab)?.length && <p className="text-center text-4xl">No schedule available for this day</p>}
									{schedules
										.filter((schedule) => schedule.day == tab.tab)
										?.map((schedule, idx) => {
											return (
